...
!function ($) {
    ...
    TimePicker.prototype = {
        ...
        _pickerEvents: function () {
            ...
            this.picker
                .on('click', '.J_up', upFunc)
                .on('mousedown', '.J_up', (ev) => mouseDownFunc(ev, upFunc))
                .on('mouseup', '.J_up', mouseUpFunc)

                .on('click', '.J_down', downFunc)
                .on('mousedown', '.J_down', (ev) => mouseDownFunc(ev, downFunc))
                .on('mouseup', '.J_down', mouseUpFunc)
            ...
            function mouseDownFunc(ev, func) {
                clearInterval(timer)
                timer = setInterval(() => {
                    func(ev)
                }, 120)
